...
# ===== データ拡張パラメータ =====
TRAIN_EPISODES_PER_TASK = 10       # 5 → 10（データ量 8 倍：50→~400 ep）
EXPECTED_MIN_TASKS = 30            # 40 期待、閾値 30 で安全側検証

# ===== 学習ハイパラ =====
STEPS = 8000                       # 3000 → 8000（データ増加に対応）
LOG_FREQ = 200                     # 100 → 200
BATCH_SIZE = 2                     # 1 → 2（T4 で bf16 想定）
GRAD_ACCUM_STEPS = 8               # 新規（実効 batch = 16）
LEARNING_RATE = 2e-4               # 3e-4 → 2e-4（拡張時控えめ）
FINAL_LEARNING_RATE = 2e-5         # 3e-5 → 2e-5
WARMUP_STEPS = 300                 # 100 → 300

# ===== LoRA 設定拡張 =====
LORA_R = 32                        # 16 → 32
LORA_ALPHA = 64                    # 16 → 64（scaling = 2.0）
LORA_DROPOUT = 0.1                 # 新規（過学習対策）
...
for task_id in EVAL_TASK_IDS:
    ...
    rows.append(
        {
            "Task ID": task_id,
            "Task": list(selected_by_task.keys())[task_id],
            "Base (%)": base_score,
